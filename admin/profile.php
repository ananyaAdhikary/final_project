...
// Safe migration for address column
...
// Admin-wide stats (whole system, not just this admin)
$sysStats = $conn->query("SELECT
    ...
    (SELECT COALESCE(SUM(total_fare),0) FROM bookings WHERE status='confirmed') as revenue
")->fetch_assoc();
...
<title>Admin Profile — BD Bus Track</title>
...
/* STAT PILLS */
.stats-row { max-width: 900px; margin: -36px auto 0; padding: 0 20px; position: relative; z-index: 2; }
.stat-pill {
  background: white;
  border-radius: 12px;
  padding: 16px 18px;
...
.stat-pill .num { font-size: 22px; font-weight: 900; color: var(--primary); }
...
.form-ctrl:focus { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(22,163,74,.08); }
textarea.form-ctrl { resize: vertical; min-height: 80px; }
.icon-input { position: relative; }
.icon-input i { position: absolute; left: 13px; top: 14px; color: #9ca3af; font-size: 13px; }
...
.btn-green-main:hover { background: var(--primary-dark); box-shadow: 0 4px 12px rgba(22,163,74,.3); }
...
.role-badge { background: var(--primary-light); color: var(--primary-dark); padding: 4px 12px; border-radius: 20px; font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: .4px; }

/* QUICK ACTION CARDS */
.quick-link { display: block; text-align: center; padding: 18px; border: 1.5px solid var(--border); border-radius: 12px; text-decoration: none; color: var(--text); transition: all .2s; }
.quick-link:hover { border-color: var(--primary); background: #f0fdf4; }
.quick-link i { font-size: 24px; color: var(--primary); margin-bottom: 8px; display: block; }
.quick-link.danger { border-color: #fecaca; color: #dc2626; }
.quick-link.danger:hover { background: #fef2f2; }
.quick-link.danger i { color: #dc2626; }
</style>
</head>
<body>
...
<nav class="topnav">
  ...
  <div style="margin-left:auto;display:flex;align-items:center;gap:4px;">
    ...
    <a href="total_bookings.php" class="nav-a"><i class="fas fa-ticket-alt me-1"></i>Bookings</a>
    <a href="todays_bookings.php" class="nav-a"><i class="fas fa-calendar-day me-1"></i>Today</a>
...
<div class="profile-hero">
  <div class="hero-inner">
    ...
    <div>
      ...
      <div class="admin-pill"><i class="fas fa-shield-alt me-1"></i>Administrator</div>
    </div>
    <div style="margin-left:auto;text-align:right">
      ...
      <div style="color:rgba(255,255,255,.5);font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;margin-top:8px;">Admin since</div>
      <div style="color:white;font-weight:700;font-size:13px;margin-top:2px;"><?= $joined ?></div>
    </div>
  </div>
</div>

<!-- Stats Pills (system-wide) -->
<div class="stats-row">
  <div class="row g-3">
    <div class="col-3">
      <div class="stat-pill">
        <div class="num"><?= $sysStats['total_bookings'] ?? 0 ?></div>
        <div class="lbl">Total Bookings</div>
      </div>
    </div>
    <div class="col-3">
      <div class="stat-pill">
        <div class="num" style="color:#16a34a"><?= $sysStats['confirmed'] ?? 0 ?></div>
        <div class="lbl">Confirmed</div>
      </div>
    </div>
    <div class="col-3">
      <div class="stat-pill">
        <div class="num" style="color:#dc2626"><?= $sysStats['cancelled'] ?? 0 ?></div>
        <div class="lbl">Cancelled</div>
      </div>
    </div>
    <div class="col-3">
      <div class="stat-pill">
        <div class="num" style="font-size:18px">৳<?= number_format($sysStats['revenue'] ?? 0) ?></div>
          ...
          <form method="POST">
            ...
            <div class="mb-4">
              ...
              <div class="icon-input">
                ...
                <textarea class="form-ctrl" name="address" rows="2"><?= htmlspecialchars($user['address'] ?? '') ?></textarea>
              </div>
            </div>
      ...
      <div class="section-card">
        ...
        <div class="section-body">
          <div class="switch-row">
            <div>
              <div class="switch-label">New Booking Alerts</div>
              <div class="switch-sub">Notify when a new ticket is booked</div>
            </div>
            <label class="toggle"><input type="checkbox" checked><span class="slider"></span></label>
          </div>
          <div class="switch-row">
            <div>
              <div class="switch-label">System Alerts</div>
              <div class="switch-sub">Errors and important system events</div>
            </div>
            <label class="toggle"><input type="checkbox" checked><span class="slider"></span></label>
          </div>
          <div class="switch-row">
            <div>
              <div class="switch-label">Daily Email Reports</div>
              <div class="switch-sub">Summary of bookings and revenue</div>
            </div>
      ...
      <div class="section-card">
        ...
        <div class="section-body">
          <div class="info-row">
            ...
            <span class="info-val"><?= date('d M Y', strtotime($user['created_at'] ?? 'now')) ?></span>
          </div>
          <div class="info-row">
          ...
          <div class="info-row">
            ...
            <span class="role-badge" style="background:#dcfce7;color:#15803d">Active</span>
          </div>
          <div class="info-row">
      ...
      <div class="section-card">
        ...
        <div class="section-body">
          <div class="row g-3">
            ...
            <div class="col-md-3">
              <a href="total_bookings.php" class="quick-link">
                <i class="fas fa-list"></i>
                <div style="font-weight:700;font-size:13px">All Bookings</div>
              </a>
            </div>
            <div class="col-md-3">
              <a href="manage_bus.php" class="quick-link">
                <i class="fas fa-bus"></i>
                <div style="font-weight:700;font-size:13px">Manage Buses</div>
              </a>
            </div>
            <div class="col-md-3">
              <a href="../logout.php" class="quick-link danger">
